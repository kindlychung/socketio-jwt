			...
			const onJwtVerificationReady = (err, decoded) => {
				...
				const onSuccess = () => {
					...
					if (server.$emit) {
						...
						if (server.nsps) {
							console.log("server.nsps is defined: ", server.nsps)
						} else {
							console.error("server.nsps not defined")
						}
						if (socket.nsp) {
						...
						if (server.nsps[socket.nsp.name]) {
							console.log("server.nsps[socket.nsp.name]: ", server.nsps[socket.nsp.name])
						} else {
							console.error("server.nsps[socket.nsp.name] not defined")
						}
						if (server.sockets) {
						...
						if (namespace) {
							console.log("\n\n\n\n>>>>>>>>>>>>>> Get namespace from server.nsps[socket.nsp.name]");
						} else {
